 ...
 * Tests registerController() functionality with Fastify.
 */
 ...
 * Fastify returns 400 for body validation, may return 400 or 422 for query/params.
 */
...
test.group('Builder Pattern - Query Parameter Validation', (group) => {
  ...
  test('applies default values', async ({ assert }) => {
    const response = await fetch(`${baseUrl}/api/v1/users`)
    assert.equal(response.status, 200)
    // Default values are applied by Fastify
  })
